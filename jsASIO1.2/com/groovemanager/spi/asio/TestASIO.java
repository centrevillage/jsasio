			...
			if(filename == null || filename.length() == 0) System.out.println("Error: No filename given.");
				...
				if(!file.exists()) System.out.println("Error: File " + filename + " doesn´t exist.");
					...
					try {
						...
						ASIOAudioFormat targetFormat = ASIOAudioFormat.convertToASIOAudioFormat(is.getFormat());
						...
						if(!info.isFormatSupported(targetFormat)){
							...
							ASIOAudioFormat[] formats = ASIOAudioFormat.convertToASIOAudioFormat(info.getFormats());
							...
							for(int i = 0; i < formats.length && !supported; i++){
								...
								else System.out.println("No conversion to "+i+": " + formats[i] + " found.");
							}
							if(!supported){
