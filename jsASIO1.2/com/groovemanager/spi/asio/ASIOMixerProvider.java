...
/**
 * This class serves as ServiceProvider for accessing ASIODrivers from Java
 * Sound
 * 
 * @author Manu Robledo
 */
public class ASIOMixerProvider extends MixerProvider {
	...
	/**
	 * speed improvement enables/disables fullCheck useful for realtime use, if
	 ...
	 */
	private static HashMap<String, ASIOMixer> mixerPool = new HashMap<String, ASIOMixer>();
	/**
	 ...
	 */
	private static ArrayList<ASIOListener> asioListeners = new ArrayList<ASIOListener>();
	/**
	 * The version of the jsasio library version
	 */
	final static String LIB_VERSION = "1.2";
	/**
	...
	public Info[] getMixerInfo() {
		...
		if (drivers == null)
			drivers = asioDrivers.getDriverNames();
		// Mixer.Info-Array vorbereiten
		if (infos == null)
			infos = new Info[drivers.length];
		else
			return infos;
	 ...
	 */
	public ASIOMixer getMixer(Info info) {
		ASIOMixerInfo i;
		// Ist das überhaupt eine ASIOInfo?
		if (info instanceof ASIOMixerInfo)
	 ...
	 * 
	 * @return The ASIODriverInfo of the currently loaded driver or null if no
	 *         driver is loaded
	 */
	...
	private static boolean ASIOLoad(ASIOMixer mixer) {
		...
		if (activeMixer == mixer && mixer.getStatus() >= ASIOMixer.LOADED)
			return true;

		// load ASIO driver
		if (!asioDrivers.loadDriver(mixer.getMixerInfo().getDriverName())) {
			errorMessage = "ASIOLoad: Unable to load ASIO Mixer Driver."
					+ mixer.getMixerInfo().getDriverName();
	...
	static long ASIOInit(ASIOMixer mixer) throws ASIOError {
		if (!ASIOLoad(mixer))
			return -1;
		if (mixer.getStatus() >= ASIOMixer.INITIALIZED)
			return nextSeq();
	 ...
	 * 
	 * @param mixer
	 *            The ASIO driver to prepare
	 * @param infos
	 *            The ASIOBufferInfo objects that should be filled
	 * @param buffersize
	 *            The buffer size in sample frames
	 * @return A sequence number for the use with ASIOExit()
	 * @throws ASIOError
	 *             If ASIOCreateBuffers() fails
	 */
	static long ASIOPrepare(ASIOMixer mixer, ASIOBufferInfo[] infos,
			int buffersize) throws ASIOError {
		long seq = ASIOInit(mixer);
		if (seq == -1)
			return -1;
		if (mixer.getStatus() >= ASIOMixer.PREPARED)
			return seq;
	 ...
	 * 
	 * @param mixer
	 *            The ASIO driver to start
	 * @param infos
	 *            The ASIOBufferInfos to be filled
	 * @param buffersize
	 *            The buffersize
	 ...
	 *             If ASIOStart() fails
	 */
	static long ASIOStart(ASIOMixer mixer, ASIOBufferInfo[] infos,
			int buffersize) throws ASIOError {
		long seq = ASIOPrepare(mixer, infos, buffersize);
		if (seq == -1)
			return -1;
		if (mixer.getStatus() == ASIOMixer.RUNNING)
			return seq;
	 ...
	 * @return true If the Mixer was already stopped before or if the Mixer
	 *         could be stopped
	 * @throws ASIOError
	 *             If ASIOStop() failed
	 */
	 ...
	 * @return true if this Mixer´s buffers had already been disposed or if the
	 *         dispose succeeded
	 * @throws ASIOError
	 *             If ASIODisposeBuffers failed
	 */
	 ...
	 *            call, ASIOExit() will really be called, otherwise not
	 */
	static void ASIOExit(ASIOMixer mixer, long seq) {
		if (mixer == activeMixer && mixer != null
				&& mixer.getStatus() >= ASIOMixer.LOADED && seq == initSeq)
			ASIOUnLoad(mixer);
	}
	...
	private static boolean bufferSizeChanged(int newSize) {
		...
		for (Iterator<ASIOListener> iter = asioListeners.iterator(); iter
			...
			if (listener.supportsBufferSizeChange())
				accepted = accepted
						|| listener.bufferSizeChanged(activeMixer, newSize);
		}
		return accepted;
	}
	...
	private static boolean resyncRequest() {
		...
		for (Iterator<ASIOListener> iter = asioListeners.iterator(); iter
			...
			ASIOListener listener = iter.next();
			if (listener.supportsResyncRequest())
				accepted = accepted || listener.resyncRequest(activeMixer);
		}
		return accepted;
	}
	...
	private static boolean latenciesChanged() {
		...
		for (Iterator<ASIOListener> iter = asioListeners.iterator(); iter
			...
			ASIOListener listener = iter.next();
			if (listener.supportsLatenciesChanged())
				accepted = accepted || listener.latenciesChanged(activeMixer);
		}
		return accepted;
	}
	...
	private static int asioMessage(int selector, int value) {
		if (selector == ASIOStaticFunctions.MESSAGE_SELECTOR_kAsioSelectorSupported) {
			if (selectorSupported(value))
				return 1;
			else
				return 0;
		} else if (selector == ASIOStaticFunctions.MESSAGE_SELECTOR_kAsioEngineVersion)
			return 2;
			...
			if (latenciesChanged())
				...
				return 0;
		} else if (selector == ASIOStaticFunctions.MESSAGE_SELECTOR_kAsioSupportsTimeInfo)
			return 1;
		else if (selector == ASIOStaticFunctions.MESSAGE_SELECTOR_kAsioSupportsTimeCode)
			return 0;
		else
			return 0;
	}
