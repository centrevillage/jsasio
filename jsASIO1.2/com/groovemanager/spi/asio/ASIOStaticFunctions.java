...
public final class ASIOStaticFunctions {
	 ...
	 * Wrapper for ASIOGetLatencies()
	 * 
	 * @return An int-Array with the input latency on position [0] and the
	 *         output latency on position [1] (in sample frames)
	 * @throws ASIOError
	 *             If ASIOGetLAtencies() fails
	 */
	 ...
	 * 
	 * @return The minimum buffer size supported by the currently loaded driver
	 *         in sample frames
	 * @throws ASIOError
	 *             If ASIOGetBufferSize() fails
	 */
	 ...
	 * Wrapper for ASIOGetBufferSize()
	 * 
	 * @return The maximum buffer size supported by the currently loaded driver
	 *         in sample frames
	 * @throws ASIOError
	 *             If ASIOGetBufferSize() fails
	 */
	 ...
	 * Wrapper for ASIOGetBufferSize()
	 * 
	 * @return The preferred buffer size of the currently loaded driver in
	 *         sample frames
	 * @throws ASIOError
	 *             If ASIOGetBufferSize() fails
	 */
	 ...
	 * Wrapper for ASIOGetBufferSize()
	 * 
	 * @return The granularity of the buffer size of the currently loaded driver
	 *         in sample frames, on return will hold the granularity at which
	 *         buffer sizes may differ. Usually, the buffer size will be a power
	 *         of 2; in this case, granularity will hold -1 on return, signaling
	 *         possible buffer sizes starting from minSize, increased in powers
	 *         of 2 up to maxSize. 
	 * @throws ASIOError
	 *             If ASIOGetBufferSize() fails
	 */
	 ...
	 * Wrapper for ASIOGetBufferSize()
	 * 
	 * @return An int-Array with the minimum buffer size at position [0], the
	 *         maximum buffer size az position [1], the preferred buffer size at
	 *         position [2] and the buffer size granularity at position [3].
	 * @throws ASIOError
	 *             If ASIOGetBufferSize() fails
	 */
	...
	static boolean ASIOCanSampleRate(double sampleRate) throws ASIOError {
		...
		if (code == ASIOError.ASE_OK)
			return true;
		else if (code == ASIOError.ASE_NoClock)
			return false;
		else
			ASIOError.throwASIOError(code);
		return false;
	 ...
	 * 
	 * @param channel
	 *            The number of the channel to query (zero-based)
	 * @param isInput
	 *            true if querying an input channel, false otherwise
	 * @return An ASIOChannelInfo instance with infos about that channel
	 * @throws ASIOError
	 *             If ASIOGetChannelinfo() fails
	 */
	static ASIOChannelInfo ASIOGetChannelinfo(int channel, boolean isInput)
			throws ASIOError {
		// ASIOMixerProvider provider = new ASIOMixerProvider();
		// Mixer.Info[] minfo = provider.getMixerInfo();
		// Mixer.Info asioMixerInfo = minfo[0];
		// ASIOMixer asioMixer = provider.getMixer(asioMixerInfo);
		// long seq = provider.ASIOInit(asioMixer);
		// try {
		...
		return info;
		// } catch (ASIOError e) {
		// provider.ASIOExit(asioMixer, seq);
		// throw e;
		// }
	}
	...
	private static native ByteBuffer createBuffer(long infoPointer, int index,
			int capacity);
	...
	/**
	 * Create two ByteBuffer objects from the buffer half addresses specified in
	 * the ASIOBufferInfo
	 * 
	 * @param info
	 *            The ASIOBufferInfo gotten from ASIOCreateBuffers()
	 * @param asiobuffersize
	...
	static ByteBuffer[] createBuffers(ASIOBufferInfo info, int asiobuffersize) {
		return new ByteBuffer[] {
				createBuffer(info.getPointer(), 0, asiobuffersize),
				createBuffer(info.getPointer(), 1, asiobuffersize) };
	}
